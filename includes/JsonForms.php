...
class JsonForms {
	 ...
	 */
/*
	public static function getProcessedSchema( $schema ) {
		...
		return $metadata->processedSchema;
	}
*/
	/**
	 * @param mixed $data
...
/*
		// @see BaseRender -> getSchemaMap
		// @TODO move to BaseRender
		$processedSchema = self::getProcessedSchema( $allParameters['schema'] );

		$schemaMap = $slotMetadata->schemaMap;
		$mappedSchema = DataSchemaProcessor::getPathToSchemaMap( $schemaMap, $processedSchema );

		if ( !self::checkData( $json, $mappedSchema ) ) {
			return \JsonForms::printError( $parserOutput, 'jsonforms-parserfunction-error-data-mismatch' );
		}
*/
