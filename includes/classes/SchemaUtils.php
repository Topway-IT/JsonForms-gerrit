...
class SchemaUtils {
	...
	/**
	 * Recursively merges source object into target object while preserving target's scalar values.
	 * 
	 * Arrays are merged element-wise, objects are merged by property, and scalars from target are preserved.
	 * Empty objects in target are replaced by source values. Properties only in source are added.
	 *
	 * @param object|array|null $target The target object/array (values are preserved)
	 * @param object|array|null $source The source object/array (provides missing values)
	 * @return object|array|null The merged result
	 */
	public static function mergeObjectsRecursive( $target, $source ) {
		...
		// Arrays: replace but recursively merge elements
		if ( is_array( $target ) && is_array( $source ) ) {
			$result = [];
			$maxCount = max( count( $target ), count( $source ) );
			for ( $i = 0; $i < $maxCount; $i++ ) {
				$targetItem = isset( $target[$i] ) ? $target[$i] : null;
				$sourceItem = isset( $source[$i] ) ? $source[$i] : null;
				$result[$i] = self::mergeObjectsRecursive( $targetItem, $sourceItem );
			}
			return $result;
		}

		// Objects: preserve target scalars, merge nested
		if ( is_object( $target ) && is_object( $source ) ) {
			$result = clone $target;
			foreach ( get_object_vars( $source ) as $property => $value ) {
				if ( !property_exists( $result, $property ) ) {
					$result->$property = $value;
				} elseif ( is_array( $result->$property ) || is_array( $value ) ) {
					$result->$property = self::mergeObjectsRecursive( $result->$property, $value );
				} elseif ( is_object( $result->$property ) && is_object( $value ) ) {
					$result->$property = self::mergeObjectsRecursive( $result->$property, $value );
				}
				// Scalars preserve target - do nothing
			}
			return $result;
		}
