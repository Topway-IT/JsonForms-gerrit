...
class SubmitForm {
	 ...
	 * @return array
	 */
	protected function postProcessJsonData( $json, $structuredValue, &$errors ) {
		$thisClass = $this;
		$callback = static function ( &$parent, $key, $value, $pathArr ) use (
			...
			if (
				...
				isset( $structuredValue->$path->schema->{'x-runtime-only'} ) &&
				$structuredValue->$path->schema->{'x-runtime-only'} === true
			) {
			...
			if (
				isset( $structuredValue->$path ) &&
				isset( $structuredValue->$path->filekey )
			) {
				$user = $thisClass->user;
				$filekey = $structuredValue->$path->filekey;
				$filename = $structuredValue->$path->value;
				$comment = '';
				$text = '';
				$watch = false;
				$tags = [];
