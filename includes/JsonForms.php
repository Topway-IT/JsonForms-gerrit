...
class JsonForms {
	...
	public static function parserFunctionRender( Parser $parser, ...$argv ) {
		...
		if ( !empty( $named['path'] ) ) {
			$json = SchemaUtils::getValueByPath( $json, $named['path'] );
		}
		...
		$schema = null;
		if ( $metadata && property_exists( $metadata, 'slots' ) && is_object( $metadata->slots ) ) {
			$slotKey = SLOT_ROLE_JSONFORMS_DATA;
			
			if ( property_exists( $metadata->slots, $slotKey ) && is_object( $metadata->slots->$slotKey ) ) {
				$slot = $metadata->slots->$slotKey;

				if ( property_exists( $slot, 'processedSchema' ) ) {
					$processedSchema = $slot->processedSchema;

					if ( is_object( $processedSchema ) && empty( (array)$processedSchema ) ) {
						$processedSchema = null;
					}
				}
				
				if ( property_exists( $slot, 'schema' ) ) {
					$schema = $slot->schema;
				}
			}
		}
		...
		$templateRender = new TemplateRender( $user, $title, $named, $processedSchema );
...
*/
	...
	public static function processSchema( $output, $schema ) {
		...
		$callback = static function ( &$parent, $key, &$value, $pathArr ) use (
			...
			// strip writeOnly
			if ( $isObject ) {
				if ( property_exists( $value, 'writeonly' ) && $value->writeonly === true ) {
					unset( $value->writeonly );
				}
				if ( property_exists( $value, 'writeOnly' ) && $value->writeOnly === true ) {
					unset( $value->writeOnly );
				}
			...
			foreach ( $wikitextKeys as $k => $v ) {
				...
				switch ( $format ) {
					...
					default:
						if ( !is_array( $fieldValue ) ) {
							// *** this will cause a double escape
							// in conjunction with InfoboxRender' htmlspecialchars
	...
	public static function prepareFormData( $output, $data ) {
		...
		if ( !empty( $data->editorOptions ) ) {
			$pathArr = explode( ":", $data->editorOptions );
			$filePath = __DIR__ . "/../data/MediaWiki/" . $pathArr[1] . ".js";
			$titleText = $data->editorOptions;
...
*/
	...
	public static function appendContent( $outputPage ) {
		...
		if (
			!empty(
				(array)$metadata->slots->{SLOT_ROLE_JSONFORMS_DATA}
					->processedSchema
			)
		) {
		...
		if ( !empty( $data->infoboxTemplate ) ) {
			...
			$parameters['template'] = $data->infoboxTemplate;
			$templateRender = new TemplateRender( $user, $title, $parameters, $processedSchema );
			$templateRender->setParser( $parser );
			$ret = $templateRender->render( $obj );

		// automatic template
		} else {
			$infoboxRender = new InfoboxRender( $user, $title, $parameters, $processedSchema );
